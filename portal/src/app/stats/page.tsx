...
import { BarChart3, Globe, Zap, Users, Shield, Coins } from 'lucide-react';
...
export default function StatsPage() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      try {
        ...
        const civilization = await civRes.json();
  ...
  if (!stats) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500">Loading network telemetry...</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 backdrop-blur-xl">
            ...
            <div className="space-y-4">
              {stats.civilization.axioms.map((a: any, i: number) => (
              ...
              {stats.civilization.axioms.length === 0 && <p className="text-slate-500 text-sm py-4">No axioms established yet.</p>}
            </div>
          </section>
          ...
          <section className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8 backdrop-blur-xl">
            ...
            <div className="space-y-3">
              {stats.civilization.top_memes.map((m: any, i: number) => (
