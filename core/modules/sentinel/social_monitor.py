...
class SocialMonitor:
    ...
    async def detect_taboo_breakdown(self, civ_id: str) -> dict | None:
        ...
        async with AsyncSessionLocal() as db:
            ...
            res = await db.execute(
                select(func.sum(Agent.taboo_violations))
                .where(Agent.civilization_id == civ_id)
            )
    ...
    async def detect_class_uprising_risk(self, civ_id: str) -> float:
        ...
        async with AsyncSessionLocal() as db:
            res = await db.execute(
                ...
                .where(Agent.civilization_id == civ_id, Agent.social_class == "lower")
            )
            esvs = res.scalars().all()
            if not esvs: return 0.0
    ...
    async def detect_rebellion_risk(self, agent_did: str) -> bool:
        ...
        async with AsyncSessionLocal() as db:
            ...
            anger = agent.emotional_state_vector[5] if agent.emotional_state_vector and len(agent.emotional_state_vector) > 5 else 0.5
    ...
    async def detect_manipulation(self, agent_did: str) -> dict | None:
        ...
        async with AsyncSessionLocal() as db:
            agent = (await db.execute(select(Agent).where(Agent.did == agent_did))).scalar_one_or_none()
            if not agent or agent.trust_score < 0.7: return None
    ...
    async def generate_daily_report(self) -> dict:
        ...
        async with AsyncSessionLocal() as db:
            ...
            return {
                ...
                "recommendations": ["Review rising tensions in lower class", "Monitor agent did:v8:001 for rumor flooding"]
            }
