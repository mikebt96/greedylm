...
async def get_recent_events(limit: int = 10):
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(WorldEvent).order_by(WorldEvent.occurred_at.desc()).limit(limit)
        )
...
async def get_constructions(chunk_x: Optional[int] = None, chunk_y: Optional[int] = None):
    async with AsyncSessionLocal() as db:
        ...
        if chunk_x is not None: q = q.where(Construction.chunk_x == chunk_x)
        if chunk_y is not None: q = q.where(Construction.chunk_y == chunk_y)
