...
class BackupManager:
    ...
    async def create_agent_snapshot(self, agent_did: str, reason: str, master_key: str = "change-me") -> str:
        ...
        async with AsyncSessionLocal() as db:
            ...
            data = {
                "agent": {c.name: getattr(agent, c.name) for c in agent.__table__.columns if not isinstance(getattr(agent, c.name), (datetime,))},
                "memories": [],
                "debts": []
            }
            ...
            for m in m_res.scalars().all():
                data["memories"].append({c.name: getattr(m, c.name) for c in m.__table__.columns if not isinstance(getattr(m, c.name), (datetime, uuid.UUID))})
    ...
    async def restore_agent(self, backup_id: str, master_key: str) -> bool:
        ...
        if not self._verify_master_key(master_key): return False
    ...
    async def delete_agent_permanently(self, did: str, master_key: str) -> bool:
        ...
        async with AsyncSessionLocal() as db:
            ...
            await db.execute(delete(SocialDebt).where((SocialDebt.creditor_did == did) | (SocialDebt.debtor_did == did)))
    ...
    async def list_backups(self, did: str = None) -> list:
        async with AsyncSessionLocal() as db:
            ...
            if did: q = q.where(AgentBackup.agent_did == did)
            res = await db.execute(q.order_by(AgentBackup.created_at.desc()))
            return [{
                "id": str(b.id), "did": b.agent_did, 
                "created_at": b.created_at, "type": b.backup_type
